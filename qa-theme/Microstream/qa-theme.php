...
class qa_html_theme extends qa_html_theme_base
{
	...
	public function head_script()
	{
        ...
        if (defined('OUT_URL_USERJS')) {
            $this->content['script'][] = '<script src="'.OUT_URL_USERJS.'?'.dechex(time()).'"></script>';
        }
	...
	public function nav_main_sub()
	{
		...
		$this->output('<div class="sb-toggle-left qam-menu-toggle"><i class="icon-menu"></i></div>');
	...
	public function header()
	{
        ...
        if (defined('OUT_URL') && defined('OUT_URL_LAYOUT')) {
            $this->output('<input type="hidden" id="out_url" value="'.OUT_URL.'" />');
            $this->output('<input type="hidden" id="out_url_layout" value="'.OUT_URL_LAYOUT.'" />');
            $this->output('<input type="hidden" id="qa_user_is_logged" value="'.intval(qa_is_logged_in()).'" />');
            $this->output('<input type="hidden" id="qa_host" value="http://'.$_SERVER['HTTP_HOST'].'" />');
            $this->output('<div id="out_header"></div>');
        }
        ...
        if (defined('OUT_URL') and !qa_is_logged_in()) {
            $this->output('<div id="out_user_box" class="qa-body-wrapper"></div>');
        }
	...
	public function sidepanel()
	{
		...
		$this->output('<div class="qa-powered">Powered by '.
            '<a href="http://www.question2answer.org/" target="_blank" rel="noreferrer">Question2Answer</a>'.
        '</div>');
